  ...
  return (
    <Card style={{padding:"0px 10px"}}>
      <Form
        ...
        onFinish={onFinish}
      >
        <div style={{margin:".5rem 0rem", display:'flex', alignItems:"end", gap:"1rem"}}>
          <div style={{width:"9rem",padding:".2rem", borderRadius:"5px",border:"1px solid gray"}}>
            <img src={previewImage || "/icons/profile.png"} style={{width:"100%", objectFit:"cover"}} alt="Profile"/>
          </div>
          <input onChange={handleFileSelected} ref={fileInputRef} style={{display:"none"}} type="file" accept="/*image/"/>
          <Button size="small" onClick={()=> fileInputRef.current.click()} style={{marginTop:".5rem"}} ><UploadOutlined/>Upload</Button>
        </div>
